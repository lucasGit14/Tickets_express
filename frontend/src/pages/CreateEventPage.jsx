      ...
      <div className="min-h-screen bg-slate-50 py-10 px-6">
        <div className="max-w-2xl mx-auto bg-white rounded-2xl border border-slate-200/60 shadow-xl p-8 space-y-6">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">Preencha as informações para publicar seu evento</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Preço do Ingresso (R$)</label>
                <input
                    type="number"
                    step="0.01"
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Local do Evento</label>
              <input
                  type="text"
                  value={formData.location}
                  onChange={(e) => setFormData({ ...formData, location: e.target.value })}
                  placeholder="Ex: Arena Central - Av. Principal, 100"
                  required
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-violet-600 text-sm"
              />
            </div>
